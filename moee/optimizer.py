...
def perform_BO_iteration(Xtr, Ytr, f, acq_func, verbose=False, acquisition_args=None):
    ...
    while N_FAILED < MAX_FAILED:
        try:
            ...
            plot_gp(model)
            # plot_gp_and_func(model, Xtr[-1], Ytr[-1])
...
if __name__ == "__main__":
    ...
    # MOEE
    all_type2 = ['topsis', 'PF', 'exploit', 'random', 'switch']
    for name in problem_names:
        for i in range(1, 31):
            for type2 in all_type2:
                perform_experiment(
                    name,  # problem name
                    ...
                    acquisition_args={"type": "UCB", "type2": type2, "explore": 0},  # acq func args
                )
    ...
    # baseline algorithms
    methods = ['PI_nsga2', 'EI', 'LCB', 'PI', 'EI_LBFGS', 'UCB_LBFGS']
    for method in methods:
        for name in problem_names:
            for i in range(1, 31):
                perform_experiment(
    ...
    for name in problem_names:
        for i in range(1, 31):
            perform_experiment(
                ...
                continue_runs=True,  # resume runs
                save=True,  # whether to save the run
                acquisition_args={"type": "EI", "type2": "topsis", "explore": 0},  # acq func args
            )
