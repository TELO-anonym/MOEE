...
def get_topsis(X_front, acf1_acf2_front):
...
class PI_nsga2(ParetoFrontOptimiser):
    """Selects the point on a GPy model's Pareto front that maximises PI.
    ...
    def __call__(self, model):
        ...
        ei = standard_acq_funcs_minimize.PI(mu, sigma, y_best=np.min(model.Y))
...
class MOEE(BaseOptimiser):
    def __call__(self, model):
        ...
        if self.acquisition_args["type2"] == "exploit":
            ...
            Ymin = np.min(Ytr)
            Ynew = model.Y[-1]
            N_Ynew = np.abs(Ynew - Ymin) / (Ymax - np.min([Ymin, Ynew]))
            print("dif:", N_Ynew)
            if N_Ynew < 0.1**4:
            ...
            if acf1_acf2_front.shape[0] != 1:
                if self.acquisition_args['explore'] >= np.sqrt(dim):
                    ...
                    tops = get_topsis(X_front, acf1_acf2_front)
